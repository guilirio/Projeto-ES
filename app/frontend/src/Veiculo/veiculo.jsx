...
import './veiculo.css';
// Se você tiver o logo na pasta assets, ajuste o caminho ou comente a linha se não tiver a imagem
import logoTrio from '../assets/logo.svg'; 

// Ícones SVG (Mesmos do Dashboard/Clients)
...
const Veiculo = ({ onLogout }) => {
  ...
  // Categorias (Mock para select, idealmente viria do backend)
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ modelo: '', quilometragem: '', marca: '', Categoria_id: '', placa: '', chassi: '', situacao: '' });
        setIsCreating(false);
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="main-content">
          ...
          <div className="header-welcome">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h2 style={{ margin: 0, fontSize: '1.5rem', color: '#333' }}>
                {isCreating ? 'Novo veículo' : 'Todos os veículos'}
              </h2>
            </div>
            <p style={{ margin: 0, color: '#666', fontSize: '0.9rem' }}>
              {isCreating 
                ? 'Criar um novo veículo' 
                : 'Visualizar, pesquisar e adicionar novos veículos'}
            </p>
          </div>
        ...
        {/* --- RENDERIZAÇÃO CONDICIONAL (Lista ou Form) --- */}
        {isCreating ? (
          /* === FORMULÁRIO DE CADASTRO === */
          <div className="client-form-container">
             {/* Botão Voltar "Interno" (opcional, visualmente igual ao design do cadastro de veículo) */}
            <div 
            ...
            <form className="client-form" onSubmit={handleSave}>
              <h3 style={{ margin: '0 0 20px 0', color: '#333' }}>Adicionar veículo</h3>
          ...
          <>
            <div className="clients-filters-card">
              <div className="filter-item search-box">
                <label>Pesquisa rápida de um veículo</label>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={v.id}>
                      ...
                      <td>
                        <span className={`status-badge ${getStatusClass(v.situacao)}`}>
                          {formatStatus(v.situacao)}
                        </span>
                      </td>
